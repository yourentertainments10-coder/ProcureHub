...
| `register team` (also: `team`, `update team`, `purchase team`) then the Excel (Name + WhatsApp number per row) | Purchase-team update: listed members are ADDED (or their name/number corrected); members not in the sheet are KEPT. Team members receive every generated PO. Bot replies with what changed + the full current team. Caption `register team` on the file works in one step too. |
| `remove team` (also: `remove team member`, `remove from team`) then the Excel of members to remove | Removes ONLY the listed members (matched by number, or by name if no number). Everyone else stays. Bot replies with who was removed + the remaining team. |
...
- **Purchase-team registry** (`register team` on WhatsApp → Excel of Name + Number; listed members are added/corrected, unlisted members kept; `remove team` deletes listed members; audited): these members receive every PO **and every allocation / reallocation workbook** (`integrations/whatsapp/recipients.internal_file_recipients` — founder numbers + team, deduplicated; used by `allocation_batch` and `topup_output`). These workbooks list every vendor's offers side by side, so they are internal-only: no vendor or customer is ever a recipient.
...
Future pairs:
python -m backend.scripts.link_part_numbers <number-1> <number-2>