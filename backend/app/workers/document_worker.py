...
def _handle_incoming_whatsapp_text(message: IncomingWhatsAppText) -> None:
    ...
    # SALES TEAM stock question (Founder, 3 Sep 2026). Checked BEFORE the
    ...
    if sales_member is not None:
        ...
        # never mistaken for a fresh query.
        request = sales_confirm.parse_confirm(message.text)
        if request.is_confirm:
            _confirm_sales_quote_safe(message.sender, sales_member, request)
            return
        if sales_confirm.is_cancel(message.text):
            _cancel_sales_quote_safe(message.sender)
            return
        if sales_query.looks_like_stock_query(message.text):
            _answer_stock_query_safe(message.sender, message.text, sales_member)
            return
        logger.info(
            "WhatsApp text from sales member %s (%s) is not a stock query: %r",
            sales_member,
            message.sender,
            message.text,
        )
        send_reply_safe(
            message.sender,
            "Send the part number(s) to check stock, e.g.\n\n"
            "41341M68P00\n75700TF0901 x 4",
        )
        return
    ...
    # the members inline, in which case it is applied immediately.
    if daily_stock.is_admin_sender(message.sender):
        if sales_team.is_remove_sales_command_text(message.text):
            with get_session() as session:
                command_store.set_command(
                    message.sender, sales_team.REMOVE_SALES_COMMAND_KEY, session
                )
            send_reply_safe(
                message.sender,
                "Send the sales members to REMOVE — an Excel, or typed here "
                "(Name + number). Everyone else stays.",
            )
            return
        if sales_team.is_sales_command_text(message.text):
            with get_session() as session:
                command_store.set_command(
                    message.sender, sales_team.SALES_COMMAND_KEY, session
                )
            send_reply_safe(
                message.sender,
                "Send the sales team list now — an Excel, or type it here. "
                "Either layout works:\n\n"
                "Amit Kumar 9217030418\n"
                "Bhawna 9217030382\n\n"
                "…or paste the names first and the numbers after, straight "
                "from your sheet.",
            )
            return

        with get_session() as session:
            awaiting_sales = sales_team.has_pending_sales_command(
                message.sender, whatsapp_settings.grouping_window_minutes, session
            )
            removing_sales = sales_team.has_pending_remove_sales_command(
                message.sender, whatsapp_settings.grouping_window_minutes, session
            )
        if awaiting_sales or removing_sales:
            rows = sales_team.parse_member_text(message.text)
            if rows:
                _apply_sales_team_safe(message.sender, rows, remove=removing_sales)
                return
...
def _answer_stock_query_safe(sender: str, text: str | None, member: str) -> None:
    """Answer a sales member's stock question and REMEMBER the answer, so a
    one-word confirm can turn it into an order. Never raises."""
    try:
        with get_session() as session:
            answers = sales_query.check_stock(text, session)
            quote = (
                sales_confirm.record_quote(sender, member, answers, session)
                if any(a.is_available for a in answers)
                else None
            )
            reference = quote.reference if quote is not None else None
        logger.info(
            "Stock query from sales member %s: %d part(s) checked (quote=%s).",
            member,
            len(answers),
            reference,
        )
        reply = sales_query.build_reply(answers, member)
        if reference:
            reply += (
                f"\n\n[{reference}] To order these, reply:\n"
                f'"confirm <customer name>"'
            )
        send_reply_safe(sender, reply)
    except Exception:  # noqa: BLE001 -- a query must never kill the worker
        logger.exception("Could not answer the stock query from %s.", sender)
        send_reply_safe(
            sender, "Sorry — could not check stock just now. Please try again."
        )


def _age_text(minutes: int) -> str:
    """'40 minutes' / '2.5 hours' -- so the member knows WHY it was
    re-checked rather than just being asked again."""
    if minutes < 90:
        return f"{max(minutes, 1)} minute(s)"
    hours = minutes / 60
    return f"{hours:.0f} hours" if hours >= 10 else f"{hours:.1f} hours"


def _cancel_sales_quote_safe(sender: str) -> None:
    """Drop the member's open quote."""
    try:
        with get_session() as session:
            quote = sales_confirm.find_confirmable(sender, None, session)
            if quote is None:
                send_reply_safe(sender, "Nothing pending to cancel.")
                return
            quote.status = "EXPIRED"
            reference = quote.reference
        send_reply_safe(sender, f"Cancelled [{reference}]. No order was placed.")
    except Exception:  # noqa: BLE001
        logger.exception("Could not cancel the sales quote for %s.", sender)


def _confirm_sales_quote_safe(sender: str, member: str, request) -> None:
    """Turn a sales member's stock check into a real customer order.

    Re-checks the stock FIRST: between the question and this confirmation
    another order may have taken the same parts, and promising stock that is
    already gone is exactly what this system exists to prevent. Never
    raises."""
    try:
        with get_session() as session:
            quote = sales_confirm.find_confirmable(sender, request.reference, session)
            if quote is None:
                send_reply_safe(
                    sender,
                    "I don't have a recent stock check to confirm. Send the part "
                    "number(s) again and I'll check availability first.",
                )
                return

            # TOO OLD TO ORDER FROM DIRECTLY (Founder, 3 Sep 2026): rather
            # than discard the parts and make them retype, look the stock up
            # AGAIN, show today's real figures, and ask them to confirm
            # THOSE. The order is never placed on the same message that
            # refreshed the numbers -- they must see them first.
            if sales_confirm.is_stale(quote):
                fresh_answers, _ = sales_confirm.recheck(quote, session)
                reference = quote.reference
                age_minutes = int(
                    (utcnow_naive() - quote.created_at).total_seconds() // 60
                ) if quote.created_at else 0
                if not any(a.is_available for a in fresh_answers):
                    quote.status = "EXPIRED"
                    send_reply_safe(
                        sender,
                        f"[{reference}] That check was {_age_text(age_minutes)} old, so "
                        "I re-checked — none of those parts are available now.",
                    )
                    return
                sales_confirm.refresh(quote, fresh_answers, session)
                if request.customer_name:
                    quote.customer_name = request.customer_name.strip()
                lines = sales_query.build_reply(fresh_answers).splitlines()
                body = "\n".join(lines[1:]).strip()
                who = (request.customer_name or quote.customer_name or "").strip()
                send_reply_safe(
                    sender,
                    f"[{reference}] That check was {_age_text(age_minutes)} old — "
                    "here is the stock RIGHT NOW:\n\n"
                    f"{body}\n\n"
                    + (
                        f'Confirm this order for {who}? Reply "yes".'
                        if who
                        else 'Reply "confirm <customer name>" to place this order.'
                    ),
                )
                return

            # The customer must be named -- the order has to belong to
            # somebody. If they didn't say, OFFER the one they last ordered
            # for and wait for a yes.
            #
            # Deliberately offered, never applied silently: one sales person
            # serves MANY customers, so quietly reusing the last name would
            # file the order against the wrong one. Showing it still costs
            # them only a single word.
            customer = (request.customer_name or quote.customer_name or "").strip()
            if not customer:
                reference = quote.reference
                parts = len(quote.parts or [])
                suggestion = sales_team.recall_customer(
                    sender,
                    session,
                    within_hours=whatsapp_settings.sales_customer_memory_hours,
                )
                if suggestion:
                    # Park it on the quote so the next "yes" picks it up --
                    # but nothing is ordered until that yes arrives.
                    quote.customer_name = suggestion
                    send_reply_safe(
                        sender,
                        f"[{reference}] {parts} part(s) ready to order.\n"
                        f'Same customer as last time — {suggestion}?\n'
                        'Reply "yes" to use it, or "confirm <other customer>".',
                    )
                    return
                ...
                send_reply_safe(
                    sender,
                    f"[{reference}] {parts} part(s) ready to order.\n"
                    'Which customer is this for? Reply "confirm <customer name>".',
                )
                return

            answers, warnings = sales_confirm.recheck(quote, session)
            usable = [a for a in answers if a.is_available]
            reference = quote.reference
            quote_id = quote.id
            order_text = sales_confirm.quote_as_text(quote)

        if not usable:
            send_reply_safe(
                sender,
                f"[{reference}] Sorry — none of those parts are available any "
                "more. Please re-check before ordering.",
            )
            return

        if warnings:
            # Say what moved, then place the order for what is still there.
            send_reply_safe(
                sender,
                f"⚠️ [{reference}] Stock changed since the check:\n"
                + "\n".join(f"• {w}" for w in warnings)
                + "\n\nPlacing the order for what is still available.",
            )

        # From here it is an ordinary customer order: the SAME typed-order
        # path a customer uses, so allocation and the purchase-team workbook
        # behave identically. The sales member never sees the vendor.
        order = customer_text_order.parse_text_order(f"{customer}\n{order_text}")
        if not order.is_order:
            send_reply_safe(
                sender, f"[{reference}] Could not build the order. Please re-send."
            )
            return
        _process_text_order(sender, order, notify=False)

        with get_session() as session:
            fresh = session.get(SalesQuote, quote_id)
            if fresh is not None:
                fresh.status = "CONFIRMED"
                fresh.customer_name = customer
                fresh.confirmed_at = utcnow_naive()
            # Offer this customer back on their next bare "confirm".
            sales_team.remember_customer(sender, customer, session)

        logger.info(
            "Sales member %s confirmed quote %s for customer %r (%d part(s)).",
            member,
            reference,
            customer,
            len(usable),
        ...
        send_reply_safe(
            sender,
            f"✅ Order confirmed for {customer} — {len(usable)} part(s).\n"
            "The purchase team has the buying list.",
        )
    except Exception:  # noqa: BLE001 -- a confirmation must never kill the worker
        logger.exception("Could not confirm the sales quote from %s.", sender)
        send_reply_safe(
            sender, "Sorry — could not place that order. Please try again."
        )


def _apply_sales_team_safe(sender: str, rows, *, remove: bool = False) -> None:
    """Apply a sales-team list and reply with exactly what changed."""
    try:
        with get_session() as session:
            if remove:
                reply, stats = sales_team.apply_removal(rows, session)
            else:
                reply, stats = sales_team.apply_update(rows, session)
            command_store.clear_command(sender, session)
        logger.info("Sales team %s: %s", "removal" if remove else "update", stats)
        send_reply_safe(sender, reply)
    except Exception:  # noqa: BLE001
        logger.exception("Could not apply the sales team list.")
        send_reply_safe(
            sender,
            "Sorry — that sales team list could not be applied. Please check "
            "the names and numbers and try again.",
        )
